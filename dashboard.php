...
$sql50 = mysqli_query($conn, 'SELECT SUM(fld_price) AS value_sum FROM tbl_booking where postdate > now() - INTERVAL 1 month'); 
$row = mysqli_fetch_assoc($sql50); 
$totalravenue = $row['value_sum'];

$sql51 = mysqli_query($conn, 'SELECT AVG(fld_price) AS avg_sum FROM tbl_booking where postdate > now() - INTERVAL 1 month'); 
$row = mysqli_fetch_assoc($sql51); 
$avgsales = $row['avg_sum'];  
